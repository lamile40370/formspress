<?php

namespace FlowForms\Modules\Actions\Services\Actions\Integrations;

use FlowForms\Modules\Actions\Services\AbstractAction;

class MailerPressAction extends AbstractAction {

	public function get_id(): string {
		return 'mailerpress';
	}

	public function get_label(): string {
		return __( 'MailerPress — Add contact', 'flowforms' );
	}

	public function get_icon(): string {
		return '<?xml version="1.0" encoding="UTF-8" standalone="no"?>
<!DOCTYPE svg PUBLIC "-//W3C//DTD SVG 1.1//EN" "http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd">
<svg width="100%" height="100%" viewBox="0 0 512 512" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xml:space="preserve" xmlns:serif="http://www.serif.com/" style="fill-rule:evenodd;clip-rule:evenodd;stroke-linejoin:round;stroke-miterlimit:2;">
    <g id="Icon" transform="matrix(2.37638,0,0,2.37638,-430.681,-963.181)">
        <path d="M256.27,505.42C256.26,505.418 256.25,505.417 256.24,505.417C256.152,505.417 256.08,505.489 256.08,505.577L256.08,505.58C256.28,520.713 256.377,535.863 256.37,551.03C256.36,556 257.01,559.28 261.17,561.37C262.09,561.83 263.697,562.053 265.99,562.04C293.957,561.88 319.703,561.9 343.23,562.1C350.323,562.16 353.863,558.543 353.85,551.25C353.823,543.01 353.78,517.927 353.72,476C353.72,472.753 353.517,470.697 353.11,469.83C352.223,467.93 350.993,466.423 349.42,465.31C348.333,464.543 346.517,464.16 343.97,464.16C306.977,464.173 269.987,464.177 233,464.17C229.58,464.17 227.09,465.413 225.53,467.9C224.61,469.367 224.163,471.69 224.19,474.87C224.363,499.023 224.54,522.383 224.72,544.95C224.72,545.316 224.451,545.631 224.09,545.69C220.04,546.34 216.27,546.38 212.83,543.79C208.883,540.81 206.91,536.72 206.91,531.52C206.903,495.88 206.897,475.88 206.89,471.52C206.87,462.313 210.62,455.097 218.14,449.87C223.65,446.03 228.33,445.25 235.56,445.26C283.373,445.287 319.107,445.297 342.76,445.29C355.047,445.283 363.82,450.87 369.08,462.05C370.113,464.23 370.633,466.837 370.64,469.87C370.82,512.15 370.95,540.31 371.03,554.35C371.077,562.237 367.97,568.863 361.71,574.23C355.9,579.21 350.57,580.73 342.41,580.74C337.717,580.753 313.733,580.78 270.46,580.82C263.26,580.827 258.573,580.44 256.4,579.66C248.47,576.83 241.41,569.27 239.93,560.69C239.777,559.77 239.687,556.3 239.66,550.28C239.54,525.507 239.54,502.183 239.66,480.31C239.667,479.823 239.91,479.58 240.39,479.58L255.5,479.58C256.149,479.58 256.768,479.857 257.2,480.34L288.64,515.15C288.65,515.162 288.661,515.173 288.672,515.184C289.027,515.542 289.618,515.541 289.98,515.181C289.99,515.171 290,515.161 290.01,515.15L321.04,480.77C321.689,480.057 322.604,479.65 323.56,479.65L336.88,479.65C337.259,479.65 337.57,479.961 337.57,480.34L337.57,544.81C337.57,545.077 337.44,545.21 337.18,545.21L321.99,545.21C321.738,545.21 321.53,545.002 321.53,544.75L321.53,506.3C321.53,505.407 321.24,505.297 320.66,505.97L290.05,541.28C289.923,541.428 289.737,541.514 289.541,541.514C289.365,541.514 289.195,541.444 289.07,541.32C277.94,530.31 267.95,518.45 256.88,505.8C256.7,505.593 256.497,505.467 256.27,505.42Z" style="fill:rgb(33,63,212);fill-rule:nonzero;"/>
    </g>
</svg>
';
	}

	public function get_description(): string {
		return __( 'Add or update a contact in MailerPress.', 'flowforms' );
	}

	public function get_fields(): array {
		return [
			[
				'key'         => 'email_field',
				'type'        => 'field-select',
				'label'       => __( 'Email field ID', 'flowforms' ),
				'help'        => __( 'The form field ID holding the email address.', 'flowforms' ),
				'placeholder' => 'email',
				'allowedTypes' => [ 'email' ],
				'default'     => '',
			],
			[
				'key'     => 'first_name_field',
				'type'    => 'field-select',
				'label'   => __( 'First name field ID', 'flowforms' ),
				'help'    => __( 'Optional.', 'flowforms' ),
				'default' => '',
			],
			[
				'key'     => 'last_name_field',
				'type'    => 'field-select',
				'label'   => __( 'Last name field ID', 'flowforms' ),
				'help'    => __( 'Optional.', 'flowforms' ),
				'default' => '',
			],
			[
				'key'     => 'subscription_status',
				'type'    => 'select',
				'label'   => __( 'Subscription status', 'flowforms' ),
				'default' => 'subscribed',
				'options' => [
					[ 'value' => 'subscribed', 'label' => __( 'Subscribed', 'flowforms' ) ],
					[ 'value' => 'pending', 'label' => __( 'Pending', 'flowforms' ) ],
					[ 'value' => 'unsubscribed', 'label' => __( 'Unsubscribed', 'flowforms' ) ],
				],
			],
			[
				'key'     => 'list_ids',
				'type'    => 'multi-select',
				'label'   => __( 'Lists', 'flowforms' ),
				'help'    => __( 'Optional. MailerPress will use its default list when none are selected.', 'flowforms' ),
				'options' => $this->get_mailerpress_list_options(),
				'default' => [],
			],
			[
				'key'     => 'tag_ids',
				'type'    => 'multi-select',
				'label'   => __( 'Tags', 'flowforms' ),
				'help'    => __( 'Optional. Select one or more MailerPress tags.', 'flowforms' ),
				'options' => $this->get_mailerpress_tag_options(),
				'default' => [],
			],
			[
				'key'     => 'custom_fields',
				'type'    => 'field-mapping-repeater',
				'label'   => __( 'Custom field mappings', 'flowforms' ),
				'help'    => __( 'Optional. Each row sends the selected FormsPress field value into the MailerPress custom field key.', 'flowforms' ),
				'default' => [],
			],
		];
	}

	public function run( array $config, array $entry, array $form ): void {
		if ( ! function_exists( 'add_mailerpress_contact' ) ) {
			error_log( '[FlowForms][MailerPress] add_mailerpress_contact() is not available.' );
			return;
		}

		$email = sanitize_email( $this->get_field_value( (string) ( $config['email_field'] ?? '' ), $entry ) );
		if ( '' === $email || ! is_email( $email ) ) {
			return;
		}

		$data = [
			'contactEmail'        => $email,
			'contactFirstName'    => sanitize_text_field( $this->get_field_value( (string) ( $config['first_name_field'] ?? '' ), $entry ) ),
			'contactLastName'     => sanitize_text_field( $this->get_field_value( (string) ( $config['last_name_field'] ?? '' ), $entry ) ),
			'subscription_status' => $this->normalize_subscription_status( (string) ( $config['subscription_status'] ?? 'subscribed' ) ),
			'lists'               => $this->build_id_payload( $config['list_ids'] ?? [] ),
			'tags'                => $this->build_id_payload( $config['tag_ids'] ?? [] ),
			'custom_fields'       => $this->build_custom_fields_payload( $config['custom_fields'] ?? [], $entry ),
			'opt_in_source'       => 'formspress',
			'optin_details'       => sprintf(
				'FormsPress form: %s; entry: %s',
				(string) ( $form['title'] ?? $form['id'] ?? '' ),
				(string) ( $entry['id'] ?? '' )
			),
		];

		$result = add_mailerpress_contact( $data );

		if ( empty( $result['success'] ) ) {
			error_log( '[FlowForms][MailerPress] Contact sync failed: ' . (string) ( $result['error'] ?? $result['message'] ?? 'Unknown error.' ) );
		}
	}

	private function build_id_payload( string|array $ids ): array {
		return array_map(
			static fn( int $id ): array => [ 'id' => $id ],
			$this->parse_ids( $ids )
		);
	}

	private function parse_ids( string|array $ids ): array {
		$raw_ids = is_array( $ids ) ? $ids : explode( ',', $ids );

		return array_values( array_unique( array_filter(
			array_map(
				static fn( mixed $id ): int => max( 0, (int) ( is_scalar( $id ) ? trim( (string) $id ) : 0 ) ),
				$raw_ids
			),
			static fn( int $id ): bool => $id > 0
		) ) );
	}

	private function build_custom_fields_payload( string|array $mappings_config, array $entry ): array {
		$fields = [];
		$mappings = [];

		if ( is_array( $mappings_config ) ) {
			foreach ( $mappings_config as $mapping ) {
				if ( ! is_array( $mapping ) ) {
					continue;
				}

				$mappings[] = [
					'field_key' => (string) ( $mapping['field_key'] ?? '' ),
					'field_id'  => (string) ( $mapping['field_id'] ?? '' ),
				];
			}
		} else {
			foreach ( preg_split( '/\r\n|\r|\n/', $mappings_config ) as $line ) {
				$line = trim( (string) $line );
				if ( '' === $line || ! str_contains( $line, '=' ) ) {
					continue;
				}

				[ $field_key, $field_id ] = array_map( 'trim', explode( '=', $line, 2 ) );
				$mappings[] = [
					'field_key' => $field_key,
					'field_id'  => $field_id,
				];
			}
		}

		foreach ( $mappings as $mapping ) {
			$field_key   = sanitize_key( $mapping['field_key'] );
			$field_value = sanitize_text_field( $this->get_field_value( $mapping['field_id'], $entry ) );

			if ( '' === $field_key || '' === $field_value ) {
				continue;
			}

			$fields[ $field_key ] = $field_value;
		}

		return $fields;
	}

	private function normalize_subscription_status( string $status ): string {
		return in_array( $status, [ 'subscribed', 'pending', 'unsubscribed' ], true ) ? $status : 'subscribed';
	}

	private function get_mailerpress_list_options(): array {
		if ( ! function_exists( 'mailerpress_get_lists' ) ) {
			return [];
		}

		try {
			$lists = mailerpress_get_lists();
		} catch ( \Throwable ) {
			return [];
		}

		return array_values( array_filter( array_map(
			static function ( mixed $list ): ?array {
				$id   = is_object( $list ) ? ( $list->list_id ?? null ) : ( $list['list_id'] ?? null );
				$name = is_object( $list ) ? ( $list->name ?? '' ) : ( $list['name'] ?? '' );

				if ( empty( $id ) ) {
					return null;
				}

				return [
					'value' => (string) max( 0, (int) $id ),
					'label' => (string) ( $name ?: sprintf( __( 'List #%d', 'flowforms' ), max( 0, (int) $id ) ) ),
				];
			},
			is_array( $lists ) ? $lists : []
		) ) );
	}

	private function get_mailerpress_tag_options(): array {
		if ( ! function_exists( 'mailerpress_get_tags' ) ) {
			return [];
		}

		try {
			$tags = mailerpress_get_tags();
		} catch ( \Throwable ) {
			return [];
		}

		return array_values( array_filter( array_map(
			static function ( mixed $tag ): ?array {
				$id   = is_object( $tag ) ? ( $tag->tag_id ?? null ) : ( $tag['tag_id'] ?? null );
				$name = is_object( $tag ) ? ( $tag->name ?? '' ) : ( $tag['name'] ?? '' );

				if ( empty( $id ) ) {
					return null;
				}

				return [
					'value' => (string) max( 0, (int) $id ),
					'label' => (string) ( $name ?: sprintf( __( 'Tag #%d', 'flowforms' ), max( 0, (int) $id ) ) ),
				];
			},
			is_array( $tags ) ? $tags : []
		) ) );
	}
}
